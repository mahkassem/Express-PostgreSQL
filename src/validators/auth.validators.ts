...
const validateRegisterRequest = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!password) {
        ...
        if (!strongRegex.test(password)) {
            errorsBag.push('Password must contain at least one lowercase letter, one uppercase letter, one number and one special character')
        }
    }
